...
更加简单有效的方式是直接使用window.location.href='https://a.xxx.com/help-center/'; 来实现跳转。这种跳转方式不限制CSP安全机制。

### word加载项跨域问题

<b>假设我当前有一个word加载项，我使用加载项去加载一个第三方的页面，除了会出现跨域问题，还可能出现CSP安全机制问题；</b>

一般有三种解决方式:

1> 在mainfest.xml文件中设置AppDomain，允许加载第三方页面的域名。（未实践）
2> 在加载项的代码中，使用window.location.href='https://a.xxx.com/help-center/'; 来实现跳转。这种跳转方式不限制CSP安全机制。
3> 假设使用iframe内嵌的方式打开，那么有两种方式解决问题：a.使用代理转发解决跨域和CSP安全机制问题；b.在运维和服务器端配置CSP安全机制白名单，允许加载项加载第三方页面。在第三方页面也需要把我们的域名添加到CSP安全机制的白名单中，但是还需要额外处理跨域问题。
